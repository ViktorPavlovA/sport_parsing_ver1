...
if __name__ == '__main__':
    ...
    big_list_area = []
    try:
        x.del_folder()  # < -- if you want delete folder data
    except:
        pass
    for j in range(len(teams_list)):
        ...
        for i in range(len(years_list) - 1):
            ...
            list_score, name_list, area_list, index_pre = x.get_data(link_on_website)
            list_score = x.divide(list_score, index_pre)
            big_list_score =x.make_a_big_list(big_list_score,list_score)
            big_list_name = x.make_a_big_list(big_list_name,name_list)
            big_list_area = x.make_a_big_list(big_list_area,area_list)
            if i == len(years_list)-2: #на этом шаге нужно передать все значения для сохранения в xlsx
                x.save_xlsx(big_list_score, big_list_name, big_list_area, teams_list[j], location_file,
